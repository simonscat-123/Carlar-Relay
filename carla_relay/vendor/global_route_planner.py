# IMPORTANT / 重要：
# 本文件是 CARLA 官方 PythonAPI「全局路线规划器」的**本仓库内修改版**，
# 由 `carla_relay/vendor/grp_loader.py` 按路径优先加载，从而不依赖宿主机的
# CARLA `PythonAPI/carla/agents/navigation/global_route_planner.py` 是否被改动过。
# 若要与官方文件保持同步，请将该副本与仓库外的 CARLA 文件保持一致。
#
# 相对官方版本的差异：
#   - __init__ 新增 algorithm 与 weight_fn 参数（astar/dijkstra/bfs 及其代价回调）
#   - _path_search 依据 algorithm 选择 networkx 的 astar_path / dijkstra_path / shortest_path
# License 与官方一致（MIT）。

# Copyright 2018-2020 CVC. MIT license.
# 本副本来源：CARLA_0.9.16/PythonAPI/carla/agents/navigation/global_route_planner.py（已修改）
...
class GlobalRoutePlanner:
    ...
    def __init__(self, wmap, sampling_resolution, algorithm='astar', weight_fn=None):
        # type: (carla.Map, float, str, ...) -> None
        # algorithm: 'astar' | 'dijkstra' | 'bfs'
        # weight_fn : 可选 cost(u, v, edge) 回调，返回边代价值；为 None 时用边自带 'length'
        self._sampling_resolution = sampling_resolution
        self._algorithm = algorithm
        self._weight_fn = weight_fn or 'length'
        self._wmap = wmap
    ...
    def _path_search(self, origin, destination):
        ...
        weight = self._weight_fn          # 字符串边属性名，或 cost(u, v, edge) 回调
        alg = (self._algorithm or 'astar').lower()
        if alg == 'dijkstra':
            # 均匀代价最短路（读权重/惩罚）
            route = nx.dijkstra_path(self._graph, source=start[0], target=end[0], weight=weight)
        elif alg == 'bfs':
            # 无权最短路：最少跳数，忽略 length 与一切惩罚权重
            route = nx.shortest_path(self._graph, source=start[0], target=end[0])
        else:                              # 默认 A*
            route = nx.astar_path(
                self._graph, source=start[0], target=end[0],
                heuristic=self._distance_heuristic, weight=weight)
        route.append(end[1])
        return route
