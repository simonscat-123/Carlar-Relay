...
语义分割标签解析与映射。
除 label_semantic_level 传入的 world 参数外不依赖任何运行时状态，
可用 numpy 数组直接进行单元测试。
"""
...
from carla_relay.config import SEMANTIC_LEVELS
...
def label_semantic_level(sem_labels, level, instance=None, world=None):
    """把 CityScapes 语义标签图映射为指定等级的类别 ID 图 (H, W) int32。

    sem_labels: (H, W) CityScapes 标签数组
    instance: (sem_ids, actor_ids) 或 None（L2 不需要实例分割）
    world: CARLA world，L3 动态细分时用于反查 actor.type_id
    """
    cfg = SEMANTIC_LEVELS[level]
    result = np.full(sem_labels.shape, cfg["default"], dtype=np.int32)

    # 1) 静态标签合并（L2 / L3 均基于语义标签）
    for tid, lid in cfg["static_map"].items():
        result[sem_labels == tid] = lid

    # 2) L3 动态细分：用实例分割的 actor ID 覆盖动态参与者
    if level == "L3" and instance is not None and world is not None:
...
def colors_from_labels(labeled, level):
    """类别 ID 图 -> RGB 彩图 (H, W, 3) uint8"""
    colors = SEMANTIC_LEVELS[level]["colors"]
    h, w = labeled.shape
...
def ratios_from_labels(labeled, level):
